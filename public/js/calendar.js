...
function renderCalendar(date) {
	...
	dayCells[0] = `
	<div id="${browseDateTime.getTime()}" class="day" style="grid-column: ${date.getDay() + 1} / ${date.getDay() + 2}">
		<div class="date">
			1
		</div>
	</div>`;
	calendar.innerHTML = dayCells.join('');
	const cw = document.querySelectorAll('.day');
	const width = window.getComputedStyle(cw[0]).width;
	cw.forEach(cw => {
		cw.style.height = `${parseInt(width)}px`;
	});
	let counter = 2;
